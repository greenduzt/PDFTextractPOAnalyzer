...
namespace PDFTextractPOAnalyzer
{
    public class AwsTextractFacade
    {
        ...
        private readonly ILineItemProcessor _lineItemProcessor;

        public AwsTextractFacade(IS3Service s3Service, ITextractService textractService, ILineItemProcessor lineItemProcessor)
        {
            _s3Service = s3Service;
            _textractService = textractService;
            _lineItemProcessor = lineItemProcessor;
        }

        public async Task<Deal> UploadPdfAndExtractPOAsync(string filePath, string bucketName)
        {
            var fileName = Path.GetFileName(filePath);

            // Upload the PDF document to S3
            await _s3Service.UploadPdfAsync(filePath);

            // Start document analysis job to analyze the document
            var lineItems = await _textractService.AnalyzeDocumentAsync(bucketName, fileName);

            // Start expense analysis job to analyze the document
            var dealSummary = await _textractService.AnalyzeExpenseAsync(bucketName, fileName);

            // Fix quantity and description in line items
            dealSummary.LineItems = _lineItemProcessor.FixQtyAndDescription(lineItems);
    ...
    //            dealSummary.LineItems = FixQtyAndDescription(lineItems);
    ...
    //    private List<LineItems> FixQtyAndDescription(List<LineItems> lineItems)
    //    {
    //        for (var i = 0; i < lineItems.Count; i++) 
    //        {
    //            //Fixing qty and description
